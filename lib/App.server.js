...
App.prototype.writeScripts = function(store, dir, options, cb) {
  ...
  this.bundle(store, options, function(err, source, map) {
    ...
    app.scriptUrl = app.derbyBase + filename + '.js';
...
App.prototype.loadViews = function(filename, namespace, options) {
  ...
  if (!util.isProduction && !process.env.DERBY_RENDERER) this._watchViews(data.files, filename, namespace);
  // Make chainable
  return this;
};
...
App.prototype._loadStyles = function(filename, options) {
  ...
  if (!util.isProduction && !process.env.DERBY_RENDERER) this._watchStyles(styles.files, filename, options);
  return styles;
};
