...
App.prototype.bundle = function(store, options, cb) {
  ...
  this.derby.store.once('bundle', function(bundle) {
    ...
    var modeFilename = require.resolve('./mode');
    var modeSource = fs.readFileSync(process.env.DERBY_RENDERER ? './rendererMode.js' : './normalMode.js');
    ...
    bundle.transform(function(filename) {
      ...
      if (filename === viewsFilename) {
        source = app._viewsSource(options);
      }
      else if (filename === modeFilename){
        source = modeSource;
      }
      return !source ? through() : through(
        function write() {}
      , function end() {
          this.queue();
          this.queue(null);
