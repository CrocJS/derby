...
Model.prototype.refLive = function(dest, source) {
  ...
  function removeListeners() {
    listeners.forEach(function(listener) {
      model.removeListener('insert', listener[0]);
      model.removeListener('remove', listener[1]);
      model.removeListener('move', listener[2]);
    });
    listeners = [];
  }
  ...
  function addListeners() {
    ...
    segments.forEach(function(segment, index) {
      ...
      if (Array.isArray(arr)) {
        ...
        listeners.push([
          model.on('insert', curKey, listener),
          model.on('remove', curKey, listener),
          model.on('move', curKey, listener)
        ]);
      }
    });
  }
