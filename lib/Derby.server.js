...
if (process.env.DERBY_RENDERER) {
  ...
  if (!process.env.DERBY_SCRIPT_DIR) process.env.DERBY_SCRIPT_DIR = 'derby';
}
...
Derby.prototype.appFactory = function(options) {
  var derby = this;
  return function factory(packageName, packageEntryPoint) {
    ...
    if (packageName) {
      app.isPackage = true;
      app.coreApp = coreApps[options.name] || (coreApps[options.name] = factory());
